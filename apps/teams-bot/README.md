...
- **Testing Custom Bots**: Implement `ITeamsBot` in a new project or reuse `TeamsBotSample` as a template. Wire it into `TeamsCallSessionCoordinator.CreateTeamsBot` or exercise it via `TeamsBotTester` for offline debugging.
...
- **Docker (Windows)**: Build with `scripts/build-teams-image.ps1` (requires Windows host or CI agent). The Dockerfile lives at `apps/teams-bot/docker/Dockerfile.windows` and produces an IIS-hosted image.
- **Configuration**: Secrets and configuration are injected through the `realtime-meeting-secrets` secret and `realtime-meeting-config` ConfigMap (see `infrastructure/k8s/base`).
- **Scheduling**: The Kubernetes deployment pins the workload to Windows nodes using `nodeSelector`/`tolerations`. Create an EKS managed node group with Windows Server Core AMIs.
- **Health Probes**: Implement `/health` inside `PsiBot.Service` (for example via ASP.NET Core middleware) to satisfy readiness/liveness probes in the manifests.

## Roadmap Ideas

- Pluggable analytics (for example, sentiment, transcription, speaker attribution).
- Continuous validation of Windows container builds in CI.
