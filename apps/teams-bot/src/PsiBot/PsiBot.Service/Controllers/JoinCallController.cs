...
namespace PsiBot.Services.Controllers
{
    ...
    public class JoinCallController : ControllerBase
    {
        ...
        /// </summary>
        public JoinCallController(ITeamsCallLifecycleService callLifecycleService, IOptions<BotConfiguration> botConfiguration, IGraphLogger logger)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> JoinCallAsync([FromBody] JoinCallBody joinCallBody)
        {
            try
            {
                var call = await _callLifecycleService.JoinCallAsync(joinCallBody).ConfigureAwait(false);
