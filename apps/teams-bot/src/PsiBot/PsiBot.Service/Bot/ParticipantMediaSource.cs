namespace PsiBot.Services.Bot
{
    ...
    /// <summary>
    /// Streams incoming media frames into the PSI pipeline and exposes typed emitters for consumers.
    /// </summary>
    public class ParticipantMediaSource : ISourceComponent
    {
        private readonly Pipeline pipeline;
        private readonly TeamsCallSessionCoordinator callCoordinator;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ParticipantMediaSource"/> class.
        /// </summary>
        /// <param name="pipeline">Pipeline in which this component lives.</param>
        /// <param name="callCoordinator">Call handler.</param>
        /// <param name="logger">Graph logger.</param>
        public ParticipantMediaSource(Pipeline pipeline, TeamsCallSessionCoordinator callCoordinator, IGraphLogger logger)
        {
            this.pipeline = pipeline;
            this.callCoordinator = callCoordinator;
            this.logger = logger;
            this.logPrefix = $"[{nameof(ParticipantMediaSource)} {this.pipeline.Name}]";
        ...
        public void ProcessAudioFrame(AudioMediaBuffer audioFrame)
        {
            ...
            if (audioFrame.UnmixedAudioBuffers != null)
            {
                foreach (var buffer in audioFrame.UnmixedAudioBuffers)
                {
                    ...
                    var participant = TeamsCallSessionCoordinator.GetParticipantByMediaStreamId(this.callCoordinator.Call, buffer.ActiveSpeakerId);
                    var identity = TeamsCallSessionCoordinator.TryGetParticipantIdentity(participant);
                    if (identity != null)
                    {
        ...
        /// <summary>
        /// Processes incoming video frame.
        /// </summary>
        /// <param name="videoFrame">Video frame.</param>
        /// <param name="id">Video frame MSI.</param>
        public void ProcessVideoFrame(VideoMediaBuffer videoFrame, uint id)
        {
            ...
            var bgr = ConvertNv12ToBgr(data, videoFormat.Width, videoFormat.Height);
            sharedImage.Resource.CopyFrom(bgr);

            var streams = new Dictionary<string, (Shared<Image>, DateTime)>();
            var participant = TeamsCallSessionCoordinator.GetParticipantByMediaStreamId(this.callCoordinator.Call, id);
            var identity = TeamsCallSessionCoordinator.TryGetParticipantIdentity(participant);
            if (identity != null)
            {
        ...
        private static byte[] ConvertNv12ToBgr(byte[] array, int width, int height, int destWidth = 0, int destHeight = 0)
        {
            if (destWidth == 0)
            {
                destWidth = width;
