...
{
    /// <summary>
    /// Manages outbound and inbound media streams, bridging the Teams bot pipeline with PSI components.
    /// </summary>
    public class TeamsMediaStreamRouter : ObjectRootDisposable
    {
        ...
        /// <summary>
        /// Participants currently tracked for media routing.
        /// </summary>
        internal List<IParticipant> TrackedParticipants { get; }
        ...
        private readonly ParticipantMediaSource participantMediaSource;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="TeamsMediaStreamRouter"/> class.
        /// </summary>
        /// <param name="mediaSession">The media session.</param>
        /// <param name="callCoordinator">Call handler.</param>
        ...
        public TeamsMediaStreamRouter(
            ILocalMediaSession mediaSession,
            TeamsCallSessionCoordinator callCoordinator,
            Pipeline pipeline, 
            ITeamsBot teamsBot, 
            Exporter exporter,
            IGraphLogger logger,
            ...
            : base(logger)
        {
            ...
            this.TrackedParticipants = new List<IParticipant>();

            this.participantMediaSource = new ParticipantMediaSource(pipeline, callCoordinator, this.logger);

            if (exporter != null)
            {
                this.participantMediaSource.Audio.Parallel(
                    (id, stream) =>
                    {
                ...
                this.participantMediaSource.Video.Parallel(
                    (id, stream) =>
                    {
            ...
            this.participantMediaSource.Audio.PipeTo(teamsBot.AudioIn);
            this.participantMediaSource.Video.PipeTo(teamsBot.VideoIn);
            ...
            teamsBot.VideoOut?.Do(
                frame =>
                {
                    if (this.videoMediaSendStatus == MediaSendStatus.Active && teamsBot.EnableVideoOutput)
                    {
                        ...
                        var nv12 = this.ConvertBgraToNv12(image.ImageData, image.Width, image.Height);
        ...
        /// <summary>
        /// Gets the tracked participants.
        /// </summary>
        /// <returns>List&lt;IParticipant&gt;.</returns>
        public List<IParticipant> GetTrackedParticipants()
        {
            return this.TrackedParticipants;
        }
        ...
        private unsafe byte[] ConvertBgraToNv12(IntPtr data, int width, int height)
        {
        ...
        private void OnAudioMediaReceived(object sender, AudioMediaReceivedEventArgs e)
        {
            try
            {
                this.participantMediaSource.ProcessAudioFrame(e.Buffer);
                e.Buffer.Dispose();
            }
            catch (Exception ex)
            {
        ...
        private void OnVideoMediaReceived(object sender, VideoMediaReceivedEventArgs e)
        {
            ...
            this.participantMediaSource.ProcessVideoFrame(e.Buffer, e.Buffer.MediaSourceId);
            e.Buffer.Dispose();
        }
